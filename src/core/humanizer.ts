...
export const defaultHumanizer: Humanizer = (event) => {
  ...
  switch (event.type) {
    ...
    case 'agentfootprint.error.fatal':
      // Non-developer audience: lead with the (already humanized) error,
      // drop the internal stage id ("__root__") which means nothing to a
      // vibe-coder. The message from agentfootprint's wrapLLMError is
      // already a plain-language, actionable sentence.
