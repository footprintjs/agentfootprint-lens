...
export class LensSnapshotRecorder implements CombinedRecorder {
  ...
  subscribePayload(runner: LensSnapshotRunnerLike): Unsubscribe {
    // DELIBERATELY does NOT call `observeRunId` from these typed-event
    // handlers. The reason: agentfootprint's typed event channel and
    // its FlowRecorder channel currently expose DIFFERENT runId formats
    // for the same run (typed → `1779905645654-0000000002`,
    // FlowRecorder → `run-1779905645654-2`). If both call
    // `observeRunId`, every event from the "other" channel looks like
    // a new run and triggers `clear()` — thrashing wipes every node we
    // just pushed and the StepGraph stays empty.
    //
    // The FlowRecorder hooks (onRunStart/onSubflowEntry/etc.) are the
    // authoritative source for run-boundary detection — they fire
    // first and their runId is stable. Typed events arrive AFTER and
    // only decorate already-existing nodes, so they don't need to
    // own run-boundary detection.
