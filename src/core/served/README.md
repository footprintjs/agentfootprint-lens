...
React, every return frozen. agentfootprint 9.88.0 owns the rebuild
(`servedAt`), the record (`receiptAt`), the hashes (`receiptHash`,
`messageDigestInput`), the gap catalogue (`SERVED_GAPS`, `UNGAPPED_FIELDS`)
...
| `verify.ts` | per-field status from the law `hash(servedAt(k)) === receiptAt(k).hash`, computed ONLY with the library's `receiptHash` / `messageDigestInput`. |
...
3. **Verified means hashes agree — nothing else.** `verify` marks a row
   ...
   `messageDigestInput`, salted with the recording's own `runId`. No receipt or
   no hash → `'reconstructed'`. A hash that disagrees, or a value under the
   receipt key the library (or `receiptShape.ts`) refused
   (`cause: 'receipt-shape-rejected'`) → `'damaged'`. Tool NAMES carry no hash
   (never verified; the receipt's list is handed back for a diff). Tool
   SCHEMAS are hashed over the library's canonical JSON, which 9.88.0 does not
   export — so they stay `'reconstructed'` rather than be checked with a
   lookalike serializer.
