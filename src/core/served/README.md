...
| `receiptShape.ts` | the narrowing the lens applies past the library's own: `receiptAt` refuses a value with no numeric `basis.epoch` and promises nothing more; a receipt missing `system` / `messages` / `tools` / `params` / `cache` is refused HERE with the same `cause: 'receipt-shape-rejected'`, so no half-shape is ever dereferenced. `carriesCacheStrategy` is the receipt's own vintage discriminator (the 9.93.0 key), which decides what two absences mean. Both are typed as what they read (0.52.1): `receiptAt` hands back the library's `StoredReceipt` (`cache` and `cache.strategy` optional — agentfootprint 9.94.1), `isReceiptShaped` narrows it to `ShapedReceipt` (every container present; what `ServedRow.receipt` holds), and `carriesCacheStrategy` narrows `cache.strategy` to the string or `null` the receipt wrote — so a `.cache` read past neither guard fails to compile, never at run time. |
