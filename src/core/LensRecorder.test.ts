...
describe('LensRecorder — LLMCall run', () => {
  ...
  it('reports status "err" + clears in-flight when the run fails', async () => {
    // Provider throws mid-call (e.g. browser fetch fails). agentfootprint's
    // ErrorBridge emits error.fatal (bridged from footprintjs onRunFailed);
    // the recorder must flip status to 'err' and the live LLM boundary
    // must clear (no permanent "thinking…").
    const throwing = {
      name: 'boom',
      async complete() {
    ...
    const llm = LLMCall.create({ provider: throwing, model: 'mock' }).system('').build();
    ...
    const summary = rec.selectSummary();
    expect(summary.status).toBe('err');
    // agentfootprint's ErrorBridge humanizes the error.fatal payload for
    // non-developers — "Failed to fetch" → a plain-language sentence.
    // (The raw error still reaches run()'s caller — asserted above.)
    expect(summary.error).toMatch(/couldn't reach the ai model/i);
    // In-flight cleared → the live "thinking" line won't stick.
    expect(rec.liveState.isLLMInFlight()).toBe(false);
  });
});
