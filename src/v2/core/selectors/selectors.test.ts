...
function mkStep(overrides: Partial<StepNode> & Pick<StepNode, 'id' | 'kind'>): StepNode {
  return {
...
describe('selectAgentInstances — pattern 2: multi-primitive (Agent or any known kind)', () => {
  it('produces one instance per subflow node flagged as isPrimitiveBoundary', () => {
    const graph = mkGraph([
      mkStep({ id: 'triage', kind: 'subflow', label: 'triage', isPrimitiveBoundary: true, primitiveKind: 'Agent' }),
      mkStep({ id: 'billing', kind: 'subflow', label: 'billing', isPrimitiveBoundary: true, primitiveKind: 'Agent' }),
  ...
  it('treats LLMCall / Sequence / Parallel / Conditional / Loop subflows as primitive boundaries too', () => {
    ...
    const graph = mkGraph([
      mkStep({ id: 'classify', kind: 'subflow', label: 'classify', isPrimitiveBoundary: true, primitiveKind: 'LLMCall' }),
      mkStep({ id: 'respond', kind: 'subflow', label: 'respond', isPrimitiveBoundary: true, primitiveKind: 'LLMCall' }),
    ]);
    const agents = selectAgentInstances(graph);
    expect(agents).toHaveLength(2);
  ...
  it('IGNORES subflows without primitiveBoundary flag (auxiliary plumbing)', () => {
    ...
    const graph = mkGraph([
      mkStep({ id: 'agent-1', kind: 'subflow', label: 'agent', isPrimitiveBoundary: true, primitiveKind: 'Agent' }),
...
describe('selectFocusDetail — pattern 5: LLM step', () => {
  it('pulls reasoning + tokens + decision from the event log', () => {
    ...
    const log: readonly EventLogEntry[] = [
      ...
      mkLogEntry(1, 'agentfootprint.stream.llm_end', {
        content: 'Let me check',
        toolCallCount: 1,
        usage: { input: 20, output: 10 },
      }, 180),
      mkLogEntry(2, 'agentfootprint.agent.route_decided', {
        chosen: 'tool-calls',
...
describe('selectFocusDetail — pattern 6: tool step', () => {
  it('pulls args + result from the event log', () => {
    ...
    const log: readonly EventLogEntry[] = [
      mkLogEntry(0, 'agentfootprint.stream.tool_start', {
        toolCallId: 'c1',
        toolName: 'weather',
        args: { city: 'SF' },
      }, 200),
      mkLogEntry(1, 'agentfootprint.stream.tool_end', {
        toolCallId: 'c1',
        toolName: 'weather',
        result: '72°F, sunny',
      }, 250),
