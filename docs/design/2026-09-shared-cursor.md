...
- FOUND IN 5c: an axis can hold SEVERAL stops on one stage (the grouped
  axis: `Route 2` and `Route 3` both on `sf-route#43`, commits 37 and 38),
  so the stage name alone is ambiguous — `stepForAddress` now tries stage +
  commit first, then the stage ladder, then commit containment.
...
- [x] 5a `<Lens shared>` (0.56.0; DECIDED for adoption: one word `shared`, one type, every lens; `SharedCursor.over(positions)` is the general form; a clamped correction never moves the address; the axis-swap re-seat is skipped with an owner); HOST LANDED: neo Why/Flow on `<Lens shared>` — verified a Why move (sf-messages#58, 47 on the milestone axis) reads on Flow/Skill Graph as that exact stage (48 on the commit axis: the anchoring fact, exact lookup first)  · [x] 5b lens side (review took: axis keyed on commit count not the per-event stamp; `.at()` not in the ES2020 lib; 'undrilled' wording; +2 tests: `cursor` wins over `shared`, a node click jumps the address): `<SkillGraphDebugger shared>` (0.57.0; derives its axis from its recorder and its snap stops from its beats; FACT: `skill-route-refusal.json` has NO commit log → no axis → the beats axis stays, address untouched) · [x] 5b host: neo Skill Graph on `shared`, Data Graph's own `GraphCursor` mapped from `shared.forAxis('step')` — 118 lines of host axis arithmetic removed; verified a Skill Graph snap-stop move (sf-injection-engine#50, commit 44) read by Data Graph, Why (stop 11) and Flow as that stage; the bridge `shared.onStepChange` is now UNUSED in the first host · [x] 5c lens side (0.58.0): `<ContextView shared recorder>` on `<TimeTravel>` (the one transport), `previous` derived in the view; FACT: the view's milestone tag axis ≠ the grouped axis (15 vs 13 stops, `sf-route#43` at 37 vs 38) → a recorder buys parity · [x] 5c host: neo ContextLensTab on `<ContextView shared recorder>`, its ◀ ▶ and the drawer's last axis memo gone; 0.58.1 patched the off-axis transport (review finding). VERIFIED: a Context move (sf-route#71) read by Why as stop 15 and by Flow as the exact stage. The first host now holds NO axis arithmetic and NO bridge; `shared.onStepChange` stays for hosts still on step + report · [~] 5d Story: FACTS — the story trace (`AttTrace`, agentfootprint) carried no stage per beat and the player (agentthinkingui) held its own index; DONE agentfootprint `57198211` `AttTrace.at[i]` = the emit's runtimeStageId per beat (prompt = '') — RELEASE IS USER-RUN (→ 9.99.0); agentthinkingui 0.32.0 controlled `index`/`onIndexChange` (+0.32.1 types); neo StoryLensTab maps address ↔ beat by EXACT stage, moves through the commit-axis ladder, self-driving when `at` is absent. BLOCKER for the live link: neo pins a VENDORED agentfootprint (`9.98.0-098e458d`) without `at` — activates once neo pins ≥ 9.99.0 (user's/friend's call)
